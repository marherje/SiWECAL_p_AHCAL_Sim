...
namespace CALICE
{
  ...
  ConversionProcessor::ConversionProcessor() : Processor("ConversionProcessor")
  {
    ...
    */
    ...
    // The following two are for OUTPUT
    vector<float> FixedPosZExample;
    registerProcessorParameter("FixedPosZ_float",
                               "vector of z layer positions",
                               _FixedPosZ_float,
                               FixedPosZExample);
    
    vector<float> MIP2GeVFactorsExample;
    registerProcessorParameter("MIP2GeVFactors_float",
                               "vector of conversion factors",
                               _MIP2GeVFactors_float,
  ...
  void ConversionProcessor::init()
  {
    ...
    if(_NSlabs==9999) streamlog_out(ERROR)<<"Missing NSlabs parameter"<<endl;
    else if(_NSlabs==0) streamlog_out(ERROR)<<"NSlabs=0???"<<endl;
    ...
    /*
    vector<float> float_siThicknesses(_NSlabs);
    for (int iz = 0; iz < _NSlabs; iz++)
    {
  ...
  void ConversionProcessor::processEvent( LCEvent * evt )
  {
    ...
    for(unsigned int icol = 0; icol < _calorimInpCollections.size(); icol++)
    {
      if( std::find(cnames->begin(), cnames->end(), _calorimInpCollections.at(icol)) != cnames->end() )
      {
	    ...
	    streamlog_out(DEBUG)<<"_FixedPosZ_float size: "<<_FixedPosZ_float.size()<<", NSlabs: "<<_NSlabs<<endl;
	    
	    // Getting the slabs positions.
	    if(int(_FixedPosZ_float.size())==0) _FixedPosZ_float.push_back(this_cellPosZ);
	    else if(int(_FixedPosZ_float.size()) < _NSlabs){
	      if(this_cellPosZ > _FixedPosZ_float.at(_FixedPosZ_float.size()-1)) {
		_FixedPosZ_float.push_back(this_cellPosZ);
		sort(_FixedPosZ_float.begin(),_FixedPosZ_float.end());
	      }
	    }
		 
	    streamlog_out(DEBUG)<<"Read Cell position: ("<<this_cellPosX<<","<<this_cellPosY<<","<<this_cellPosZ<<")"<<endl;
  ...
  void ConversionProcessor::end(){
    ...
    for (int iz = 0; iz < _NSlabs; iz++)
    {
       ...
       if (_MIPFitMode == 1) {_MIP2GeVFactors_float.push_back(fitGaus(layerEnergyHistos[iz]));}             
       else if (_MIPFitMode == 2) {_MIP2GeVFactors_float.push_back(fitLandau(layerEnergyHistos[iz]));}
       else if (_MIPFitMode == 3) {_MIP2GeVFactors_float.push_back(fitLanGaus(layerEnergyHistos[iz]));}
       else
	 {
	   cout << "Invalid _MIPFitMode = " << std::to_string(_MIPFitMode) << "\n";
	   break;
    ...
    if( int(_FixedPosZ_float.size()) != 0 ) {
      cout<<"Fixed pos. Z vector: ";
      for (unsigned int i = 0; i < _FixedPosZ_float.size(); i++) {
	cout << _FixedPosZ_float.at(i) << " ";
      }
      cout << endl;
    }
    if( int(_FixedPosZ_float.size()) != _NSlabs ) streamlog_out(WARNING)<<"Couldn't find position of all layers. Check NSlabs."<<endl;

    if( int(_MIP2GeVFactors_float.size()) != 0 ) {
      cout<<"MIP2GeV vector: ";
      for (unsigned int i = 0; i < _MIP2GeVFactors_float.size(); i++) {
        cout << _MIP2GeVFactors_float.at(i) << " ";
      }
      cout << endl;
    }
    if( int(_MIP2GeVFactors_float.size()) != _NSlabs ) streamlog_out(WARNING)<<"Couldn't fit MIP2GeV for all layers. Check NSlabs."<<endl;
  ...
  float ConversionProcessor::fitGaus(TH1F* h)
  {
    ...
    streamlog_out(DEBUG) << f->GetParameter(1) << " +- " << f->GetParameter(2) << endl;
    return f->GetParameter(1);
  }

  float ConversionProcessor::fitLandau(TH1F* h)
  {
    _writeHisto = false;
    ...
    streamlog_out(DEBUG) <<"Fit Landau: "<< mpv_corrected << endl;
    return float(mpv_corrected);
  ...
  float ConversionProcessor::fitLanGaus(TH1F* h)
  {
    ...
    streamlog_out(DEBUG) <<"Fit Landau-Gauss: "<< mpv_corrected << endl;
    return float(mpv_corrected);
  }
