...
void findTH1values(TH1 * hA, TH1 * hB, TH1 * hC, TH1 * hD, float &x_min, float &x_max, float &y_max, bool &rescalable) {
  ...
  float max_x_D = hD->GetBinContent(hD->GetMaximumBin());
  
  cout<<"Max (e, mu, pi, neutron): ("<<max_x_A<<", "<<max_x_B<<", "<<max_x_C<<", "<<max_x_D<<")"<<endl;
  ...
  y_max = 1.01*max(max_x_A, max(max_x_B,max(max_x_C,max_x_D)));
  cout<<"(min x, max x, max y) = ("<<x_min<<", "<<x_max<<", "<<y_max<<")"<<endl;

  return 0;
}

void drawHistosTH1(TString varname, TH1 * hA, TH1 * hB, TH1 * hC, TH1 * hD, bool save = false) {
  float x_min = 0.;
  float x_max = 0.;
  float y_max = 0.;
  bool rescalable = false;
  findTH1values(hA, hB, hC, hD, x_min, x_max, y_max, rescalable);
...
void single_histo_4_temp(TString varname, bool save = false){
  ...
  TTree *tree_D = (TTree*)file_D->Get("ntp");
  ...
  tree_D->SetBranchAddress(varname, &value_D, &branch_D);

  int entries_A = tree_A->GetEntries();
  int entries_B = tree_B->GetEntries();
  int entries_C = tree_C->GetEntries();
  int entries_D = tree_D->GetEntries();
  cout<<"entries: "<<entries_A<<" "<<entries_B<<" "<<entries_C<<" "<<entries_D<<endl;
  ...
  TH1F *histo_D = new TH1F("hD", "hD", 100, xmin, xmax);
  ...
  for (Int_t i=0; i<entries_D; i++){
    tree_D->GetEntry(i);
    histo_D->Fill(value_D);
  }

  drawHistosTH1(varname, histo_A, histo_B, histo_C, histo_D, save);
