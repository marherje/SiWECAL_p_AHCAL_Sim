...
//   if(linebuffer=="")std::cout<<"empty"<<std::endl;
  ...
  searchedString="SourceNFile=";
  if(linebuffer.find(searchedString)!=std::string::npos){
    NFile=linebuffer.substr(searchedString.length());
    std::cout<<"found NFile"<<std::endl;
  }
  searchedString="SourceNTree=";
  if(linebuffer.find(searchedString)!=std::string::npos){
    NTree=linebuffer.substr(searchedString.length());
  }
...
if(UseFixedTrainTestSplitting)
{
  ...
  AddSample("N_Train", NFile, NTree, selTrain);
  AddSample("N_Test" , NFile, NTree, selTest );
 }
else
{
  ...
  AddSample("N",NFile,NTree,Selection);
 }
