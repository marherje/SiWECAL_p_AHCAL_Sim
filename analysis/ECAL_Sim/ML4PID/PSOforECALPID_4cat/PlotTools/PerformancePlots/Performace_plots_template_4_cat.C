...
void findTH1values(TH1F * hA, TH1F * hB, TH1F * hC, TH1F * hD, float &x_min, float &x_max, float &y_max) {
  ...
  float max_y_D = hD->GetBinContent(hD->GetMaximumBin());

  //cout<<"Max (e, mu, pi): ("<<max_y_A<<", "<<max_y_B<<", "<<max_y_C<<", "<<max_y_D<<")"<<endl;

  int b_min_A = hA->FindFirstBinAbove(0,1);
  int b_min_B = hB->FindFirstBinAbove(0,1);
  int b_min_C = hC->FindFirstBinAbove(0,1);
  int b_min_D = hD->FindFirstBinAbove(0,1);

  int b_max_A = hA->FindLastBinAbove(0,1);
  int b_max_B = hB->FindLastBinAbove(0,1);
  int b_max_C = hC->FindLastBinAbove(0,1);
  int b_max_D = hD->FindLastBinAbove(0,1);

  int b_min = min(b_min_A, min(b_min_B,min(b_min_C,b_min_D)));
  int b_max = max(b_max_A, max(b_max_B,max(b_max_C,b_max_D)));

  x_min = hA->GetBinCenter(b_min-1);
  x_max = hA->GetBinCenter(b_max+1);

  y_max = 1.01*max(max_y_A, max(max_y_B,max(max_y_C,max_y_D)));
  //cout<<"(min x, max x, max y) = ("<<x_min<<", "<<x_max<<", "<<y_max<<")"<<endl;

  return 0;
}

void drawLikelihood(TString title, TH1F * hA, TH1F * hB, TH1F * hC, TH1F * hD, bool save = false) {
  float x_min = 0.;
  float x_max = 0.;
  float y_max = 0.;

  findTH1values(hA, hB, hC, hD, x_min, x_max, y_max);
  ...
  for(int i=1; i<N_BINS+1; i++) {
    ...
    sum_D += hD->GetBinContent(i);
  }
  //cout<<"Sums: "<<sum_A<<" "<<sum_B<<" "<<sum_C<<" "<<sum_D<<endl;
  //cout<<"Integrals: "<<hA->Integral()<<" "<<hB->Integral()<<" "<<hC->Integral()<<" "<<hD->Integral()<<endl;
  ...
  float xleg=0.5;
  if(title=="XpartAX-likelihood") xleg = 0.3;
  if(title=="XpartBX-likelihood") xleg = 0.55;
  if(title=="XpartCX-likelihood") xleg = 0.55;
  TLegend *leg;
  ...
  double hA_c[N_BINS+1], hB_c[N_BINS+1], hC_c[N_BINS+1], hD_c[N_BINS+1];
  ...
  for(int i=0;i<N_BINS+1;i++){
    ...
    hD_c[i] = 100.-integral_D;
    if(hA_c[i] < 0.) hA_c[i] = 0.;
    if(hB_c[i] < 0.) hB_c[i] = 0.;
    if(hC_c[i] < 0.) hC_c[i] = 0.;
    if(hD_c[i] < 0.) hD_c[i] = 0.;
  }
  ...
  auto hD_cumulative = new TGraph(N_BINS+1,cut_axis,hD_c);
  ...
  for(int i=1; i<NPoints; i++) {
    ...
    float D_content = hD_cumulative->Eval(x);
    //cout<<"x: "<<x<<". (A, B, C) = ("<<A_content<<", "<<B_content<<", "<<C_content<<")"<<endl;
    float A_impurity = 100.*(            B_content + C_content + D_content)/(A_content + B_content + C_content + D_content);
    float B_impurity = 100.*(A_content             + C_content + D_content)/(A_content + B_content + C_content + D_content);
    float C_impurity = 100.*(A_content + B_content             + D_content)/(A_content + B_content + C_content + D_content);
    float D_impurity = 100.*(A_content + B_content + C_content            )/(A_content + B_content + C_content + D_content);
    //cout<<"x: "<<x<<". Impurity (A, B, C) = "<<A_impurity<<", "<<B_impurity<<", "<<C_impurity<<", "<<D_impurity<<")"<<endl;
    ...
    if( title=="XpartDX-likelihood" ){
      if( D_impurity < 0.01*(1+tol) ){
        h_001_y[0] = D_content;
        h_001_x[0] = x;
        found_001 = true;
        break;
  ...
  float xleg_c = 0.6;
  float yleg_c = 0.6;
  if(title=="XpartAX-likelihood") xleg_c = 0.3;
  if(title=="XpartBX-likelihood") {xleg_c = 0.65; yleg_c = 0.7;}
  if(title=="XpartCX-likelihood") {xleg_c = 0.65; yleg_c = 0.7;}
  if(title=="XpartDX-likelihood") {xleg_c = 0.65; yleg_c = 0.7;}
  TLegend *leg_c;
  ...
  if( (found_001 == true) or (found_01 == true) or (found_1 == true) or (found_10 == true)) {
    float xleg_WP = 0.6;
    float yleg_WP = 0.4;
    if(title=="XpartAX-likelihood") xleg_WP = 0.3;
    if(title=="XpartBX-likelihood") {xleg_WP = 0.65; yleg_WP = 0.5;}
    if(title=="XpartCX-likelihood") {xleg_WP = 0.65; yleg_WP = 0.5;}
    if(title=="XpartDX-likelihood") {xleg_WP = 0.45; yleg_WP = 0.5;}
    TLegend *leg_WP;
...
void drawCorrelationPlots(TH2F * hA, TH2F * hB, TH2F * hC, TH2F * hD, bool save = false) {
  ...
  TH2F * A_cor = new TH2F("XpartAX_correlation","XpartAX sample correlation between variables",nbins_x,-0.5,nbins_x+0.5,nbins_y,-0.5,nbins_y+0.5);
  TH2F * B_cor = new TH2F("XpartBX_correlation","XpartBX sample correlation between variables",nbins_x,-0.5,nbins_x+0.5,nbins_y,-0.5,nbins_y+0.5);
  TH2F * C_cor = new TH2F("XpartCX_correlation","XpartCX sample correlation between variables",nbins_x,-0.5,nbins_x+0.5,nbins_y,-0.5,nbins_y+0.5);
  TH2F * D_cor = new TH2F("XpartDX_correlation","XpartDX sample correlation between variables",nbins_x,-0.5,nbins_x+0.5,nbins_y,-0.5,nbins_y+0.5);
  ...
  for(int i = 1; i<nbins_x+1; i++){
    for(int j = 1; j<nbins_y+1; j++){
      ...
      float content_D = abs(hC->GetBinContent(i,j));
      if(content_A == 0.) content_A = 0.001;
      if(content_B == 0.) content_B = 0.001;
      if(content_C == 0.) content_C = 0.001;
      if(content_D == 0.) content_D = 0.001;
      float maxvalue = max(content_A,max(content_B,max(content_C,content_D)));
      ...
      D_cor->SetBinContent(i,j,content_D);
    }
    ...
    D_cor->GetXaxis()->SetBinLabel(i,"V"+TString::Format("%i",i));
    D_cor->GetYaxis()->SetBinLabel(i,"V"+TString::Format("%i",i));
  ...
  drawCorrelation("XpartDX", varnames, D_cor, save);
...
void Performance_plots(bool save = false){
  ...
  TH1F* A_likelihood_for_D = (TH1F*) outputfile->Get("dataloader/Method_BDT/myMVA/MVA_myMVA_Test_Sample_D_prob_for_Sample_A");
  ...
  TH1F* B_likelihood_for_D = (TH1F*) outputfile->Get("dataloader/Method_BDT/myMVA/MVA_myMVA_Test_Sample_D_prob_for_Sample_B");
  ...
  TH1F* C_likelihood_for_D = (TH1F*) outputfile->Get("dataloader/Method_BDT/myMVA/MVA_myMVA_Test_Sample_D_prob_for_Sample_C");

  TH1F* D_likelihood_for_A = (TH1F*) outputfile->Get("dataloader/Method_BDT/myMVA/MVA_myMVA_Test_Sample_A_prob_for_Sample_D");
  TH1F* D_likelihood_for_B = (TH1F*) outputfile->Get("dataloader/Method_BDT/myMVA/MVA_myMVA_Test_Sample_B_prob_for_Sample_D");
  TH1F* D_likelihood_for_C = (TH1F*) outputfile->Get("dataloader/Method_BDT/myMVA/MVA_myMVA_Test_Sample_C_prob_for_Sample_D");
  TH1F* D_likelihood_for_D = (TH1F*) outputfile->Get("dataloader/Method_BDT/myMVA/MVA_myMVA_Test_Sample_D_prob_for_Sample_D");
  ...
  TH2F* correlation_matrix_D = (TH2F*) outputfile->Get("dataloader/CorrelationMatrixSample_D");
  ...
  drawLikelihood("XpartAX-likelihood", A_likelihood_for_A, A_likelihood_for_B, A_likelihood_for_C, A_likelihood_for_D, save);
  drawLikelihood("XpartBX-likelihood", B_likelihood_for_A, B_likelihood_for_B, B_likelihood_for_C, B_likelihood_for_D, save);
  drawLikelihood("XpartCX-likelihood", C_likelihood_for_A, C_likelihood_for_B, C_likelihood_for_C, C_likelihood_for_D, save); 
  drawLikelihood("XpartDX-likelihood", D_likelihood_for_A, D_likelihood_for_B, D_likelihood_for_C, D_likelihood_for_D, save);

  drawCorrelationPlots(correlation_matrix_A, correlation_matrix_B, correlation_matrix_C, correlation_matrix_D, save);
}
