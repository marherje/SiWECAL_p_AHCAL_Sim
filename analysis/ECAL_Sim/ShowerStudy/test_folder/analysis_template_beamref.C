...
float moliere(vector<float> * hit_energy, vector<int> *hit_slab, TVectorD W_thicknesses,
  ...
  float beam_wx = 0., beam_wy = 0.;
  float beam_e = 0.;
  float beam_bary_x = 0., beam_bary_y = 0.;
  
  if((hit_energy->size() > 0) and (is_shower==true)){
    for (int j = 0; j < hit_energy->size(); j++){
      if (masked && hit_isMasked->at(j) == 1) continue;
      if (hit_slab->at(j) == 0) {
	beam_wx += hit_x->at(j) * hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
	beam_wy += hit_y->at(j) * hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
	beam_e += hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
      }
    }
    beam_bary_x = beam_wx/beam_e;
    beam_bary_y = beam_wy/beam_e;
      
    for (int j = 0; j < hit_energy->size(); j++){
      if (masked && hit_isMasked->at(j) == 1) continue;
      sume += hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
      wx += (hit_x->at(j)-beam_bary_x) * hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
      wy += (hit_y->at(j)-beam_bary_y) * hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
...
void radius_layer(float mol_per_layer[N_ECAL_LAYERS], vector<float> * hit_energy, vector<int> *hit_slab, TVectorD W_thicknesses,
  ...
  float beam_e = 0.;
  float beam_bary_x = 0., beam_bary_y = 0.;
  int nhit_layer[N_ECAL_LAYERS];
  cout<<"mol_layer debug 1"<<endl;
  if((hit_energy->size() > 0) and (is_shower==true)){
    for (int j = 0; j < hit_energy->size(); j++){
      if (masked && hit_isMasked->at(j) == 1) continue;
      if (hit_slab->at(j) == 0) {
	...
	if (mol_e >= containment * sume){
      ...
      if(mol_i<0) mol_i=0.;
      mol_per_layer[ilayer] = hit_rs.at(mol_i);
      if(nhit_layer[ilayer] < 3) mol_per_layer[ilayer] = 0.;    
...
void bary_layer(float blv[N_ECAL_LAYERS][3], vector<float> * hit_energy, vector<int> *hit_slab, 
  ...
  if((hit_energy->size() > 0) and (is_shower == true)){
	...
	for (int j = 0; j < hit_energy->size(); j++) {
	  ...
	  if (hit_slab->at(j) == 0) {
	    sume_w[ilayer] += hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
	    wx[ilayer] += (hit_x->at(j)-beam_bary_x) * hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
	    wy[ilayer] += (hit_y->at(j)-beam_bary_y) * hit_energy->at(j) * W_thicknesses[hit_slab->at(j)]/(3.5);
	  }
	}
...
void fit_gauss(TH1 * h, double &mu, double &mu_error, double &sig){
  ...
  if(xmin <0.) xmin = h->GetXaxis()->GetBinCenter(h_binmax)-1*h_std;
...
void analysis_XenergyX (string particle, bool masked=false) {
    
    // double energies[N_ENERGIES] = {1, 2, 5, 8, 10, 20, 40, 60, 80, 100, 150};
    double test_e[N_ENERGIES]={XenergyX};
    ...
    // Output filename
    TString result_name = "resolution_"+particle+"_XenergyX_result.root" ;
	...
	TH1F *h_radius90_layer_0 = new TH1F(("Radius90_layer_0_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 0 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_1 = new TH1F(("Radius90_layer_1_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 1 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_2 = new TH1F(("Radius90_layer_2_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 2 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_3 = new TH1F(("Radius90_layer_3_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 3 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_4 = new TH1F(("Radius90_layer_4_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 4 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_5 = new TH1F(("Radius90_layer_5_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 5 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_6 = new TH1F(("Radius90_layer_6_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 6 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_7 = new TH1F(("Radius90_layer_7_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 7 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_8 = new TH1F(("Radius90_layer_8_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 8 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_9 = new TH1F(("Radius90_layer_9_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 9 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_10 = new TH1F(("Radius90_layer_10_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 10 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_11 = new TH1F(("Radius90_layer_11_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 11 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_12 = new TH1F(("Radius90_layer_12_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 12 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_13 = new TH1F(("Radius90_layer_13_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 13 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
        TH1F *h_radius90_layer_14 = new TH1F(("Radius90_layer_14_" + part_string + "_" + e_str).c_str(), ("Radius 90% layer 14 " + part_string + " " + e_str).c_str(), 101, -0.5, 100.5);
