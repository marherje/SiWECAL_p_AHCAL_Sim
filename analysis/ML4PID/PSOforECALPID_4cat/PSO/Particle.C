...
std::vector<Float_t> KS_AD_Test(){
  ...
  std::cout<<"Test TTree entries: "<<test_entries<<std::endl;
  
  TH1F* test_A_histo=new TH1F("test_A_histo","test_A_histo",100,0,1);
  TH1F* test_B_histo=new TH1F("test_B_histo","test_B_histo",100,0,1);
  TH1F* test_C_histo=new TH1F("test_C_histo","test_C_histo",100,0,1);
  TH1F* test_D_histo=new TH1F("test_D_histo","test_D_histo",100,0,1);

  for(int i=0;i<test_entries;i++){
    TestTree->GetEntry(i);
    Float_t value_A=TestTree->GetBranch("myMVA")->GetLeaf("Sample_A")->GetValue(0);
    Float_t value_B=TestTree->GetBranch("myMVA")->GetLeaf("Sample_B")->GetValue(0);
    Float_t value_C=TestTree->GetBranch("myMVA")->GetLeaf("Sample_C")->GetValue(0);
    Float_t value_D=TestTree->GetBranch("myMVA")->GetLeaf("Sample_D")->GetValue(0);
    test_A_histo->Fill(value_A);
    test_B_histo->Fill(value_B);
    test_C_histo->Fill(value_C);
    test_D_histo->Fill(value_D);
  ...
  std::cout<<"Train TTree entries: "<<train_entries<<std::endl;

  TH1F* train_A_histo=new TH1F("train_A_histo","train_A_histo",100,0,1);
  ...
  TH1F* train_D_histo=new TH1F("train_D_histo","train_D_histo",100,0,1);

  for(int i=0;i<train_entries;i++){
    TrainTree->GetEntry(i);

    Float_t value_A=TrainTree->GetBranch("myMVA")->GetLeaf("Sample_A")->GetValue(0);
    Float_t value_B=TrainTree->GetBranch("myMVA")->GetLeaf("Sample_B")->GetValue(0);
    Float_t value_C=TrainTree->GetBranch("myMVA")->GetLeaf("Sample_C")->GetValue(0);
    Float_t value_D=TrainTree->GetBranch("myMVA")->GetLeaf("Sample_D")->GetValue(0);
    train_A_histo->Fill(value_A);
    train_B_histo->Fill(value_B);
    train_C_histo->Fill(value_C);
    train_D_histo->Fill(value_D);
  }
  
  for(int i=0;i<100;i++){
    Int_t bincontent_A=test_A_histo->GetBinContent(i);
    Float_t error_A=pow(bincontent_A,0.5);
    test_A_histo->SetBinError(i,error_A);
    ...
    Int_t bincontent_D=test_D_histo->GetBinContent(i);
    Float_t error_D=pow(bincontent_D,0.5);
    test_D_histo->SetBinError(i,error_D);
  }
  ...
  canvas->cd(1);
  test_A_histo->Draw();
  train_A_histo->SetLineColor(2);
  train_A_histo->Draw("same");
  canvas->cd(2);
  test_B_histo->Draw();
  ...
  canvas->cd(4);
  test_D_histo->Draw();
  train_D_histo->SetLineColor(2);
  train_D_histo->Draw("same");
  canvas->SaveAs("KS-AD-Test.png");
  */
  
  Float_t ks_a=test_A_histo->KolmogorovTest(train_A_histo, "X");
  Float_t ks_b=test_B_histo->KolmogorovTest(train_B_histo, "X");
  Float_t ks_c=test_C_histo->KolmogorovTest(train_C_histo, "X");
  Float_t ks_d=test_D_histo->KolmogorovTest(train_D_histo, "X");
  Float_t KS=TMath::Min(ks_a,TMath::Min(ks_b,TMath::Min(ks_c,ks_d)));
    
  Float_t ad_a=test_A_histo->AndersonDarlingTest(train_A_histo, "X");
  ...
  Float_t ad_d=test_D_histo->AndersonDarlingTest(train_D_histo, "X"); 
  Float_t AD=TMath::Min(ad_a,TMath::Min(ad_b,TMath::Min(ad_c,ad_d)));
...
void DoTraining(std::vector<BDTVar*> UsedVars,std::vector<BDTVar*> UnUsedVars,TString FOMType, TString FactoryString, TString PrepString, TString BWeight, TString CWeight, TString OWeight, TString NWeight, TString BTreeName, TString CTreeName, TString OTreeName, TString NTreeName,TString MethodType, TString MethodString, int particleNumber, Double_t* testFOM, Double_t* testAD, Double_t* testKS, Int_t UseFixedTrainTestSplitting, TString PlotName="NONE" ){
   std::cout<<"----------------------------------------------------------------"<<std::endl;
   ...
   if(UseFixedTrainTestSplitting == 1)
   {
     ...
     dataloader->AddTree(NTest,"N_Test",1.0,"",TMVA::Types::kTesting);
   ...
   dataloader->SetWeightExpression( NWeight,"Sample_D");
   ...
   float Total_A=TreeB->GetEntries();
   float Total_B=TreeC->GetEntries();
   float Total_C=TreeO->GetEntries();
   float Total_D=TreeN->GetEntries();
   float Total_All=Total_A+Total_B+Total_C+Total_D;
   
   float Weight_A=Total_A/Total_All;
   float Weight_B=Total_B/Total_All;
   float Weight_C=Total_C/Total_All;
   float Weight_D=Total_D/Total_All;

   double ROC_A=factory->GetROCIntegral(dataloader,"myMVA",0); //MethodString? or "myMVA"?
   double ROC_B=factory->GetROCIntegral(dataloader,"myMVA",1);
   double ROC_C=factory->GetROCIntegral(dataloader,"myMVA",2);
   double ROC_D=factory->GetROCIntegral(dataloader,"myMVA",3);

   std::cout<<"ROC Integrals(A,B,C,D): "<<ROC_A<<" | "<<ROC_B<<" | "<<ROC_C<<" | "<<ROC_D<<std::endl;
   
   double Weighted_AUC=(Weight_A*ROC_A+Weight_B*ROC_B+Weight_C*ROC_C+Weight_D*ROC_D);
   std::cout<<"FOM (Weighted ROC Integral sum): "<<Weighted_AUC<<std::endl;
   
   double Weighted_1_AUC=(ROC_A+ROC_B+ROC_C+ROC_D)/4;
...
//done reading config
  ...
  {
    DoTraining(InitialVars,OtherVars,FOMType,FactoryString,PrepString,BWeight,CWeight,OWeight,NWeight,BTreeName,CTreeName,OTreeName,NTreeName,MethodType,MethodString, particleNumber, &bufferFOM, &bufferAD, &bufferKS,UseFixedTrainTestSplitting);
  ...
  if(bool(FindBestVariables))
  {
    ...
    for(int k=0;k<nUsedVars;k++){
      ...
      DoTraining(UsedVars,UnusedVars,FOMType,FactoryString,PrepString,BWeight,CWeight,OWeight,NWeight,BTreeName,CTreeName,OTreeName,NTreeName,MethodType,MethodString, particleNumber, &bufferFOM, &bufferAD, &bufferKS,UseFixedTrainTestSplitting);
      if(bufferAD<AD)AD=bufferAD;
      if(bufferKS<KS)KS=bufferKS;
      if(bufferFOM<FOM)FOM=bufferFOM;
      }
...
//    ROCFileName+=Iteration;
    ...
    for(int nn=0;nn<RepeatTrainingNTimes+1;nn++)
    {
      DoTraining(BestVars,WorstVars,FOMType,FactoryString,PrepString,BWeight,CWeight,OWeight,NWeight,BTreeName,CTreeName,OTreeName,NTreeName,MethodType,MethodString, particleNumber, &bufferFOM, &bufferAD, &bufferKS, UseFixedTrainTestSplitting);
      if(bufferAD<AD)AD=bufferAD;
      if(bufferKS<KS)KS=bufferKS;
      if(bufferFOM<FOM)FOM=bufferFOM;
    }
