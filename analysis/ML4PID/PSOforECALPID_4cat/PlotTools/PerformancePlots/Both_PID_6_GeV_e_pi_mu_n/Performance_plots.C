...
void addProduction(){
  ...
  p1->Draw();
  p1->cd();
  TString title = "Both_PID_6_GeV_e_pi_mu_n";
...
void drawLikelihood(TString title, TH1F * hA, TH1F * hB, TH1F * hC, TH1F * hD, bool save = false) {
  ...
  auto c = new TCanvas("c_Both_PID_6_GeV_e_pi_mu_n_"+title, "c_Both_PID_6_GeV_e_pi_mu_n_"+title, 800, 800);
  c->cd();
  ...
  addProduction();
  if(save == true){
    c->SaveAs("c_Both_PID_6_GeV_e_pi_mu_n_"+title+".eps");
    c->SaveAs("c_Both_PID_6_GeV_e_pi_mu_n_"+title+".png");
  }
  ...
  auto c2 = new TCanvas("c_Both_PID_6_GeV_e_pi_mu_n_"+title+"_cut", "c_Both_PID_6_GeV_e_pi_mu_n_"+title+"_cut", 800, 800);
  c2->cd();
  ...
  if( (found_001 == true) or (found_01 == true) or (found_1 == true) or (found_10 == true)) {
    ...
    if(title=="n-likelihood") {xleg_WP = 0.45; yleg_WP = 0.5;}
    TLegend *leg_WP;
  ...
  addProduction();
  if(save == true){
    c2->SaveAs("c_Both_PID_6_GeV_e_pi_mu_n_"+title+"_cut.eps");
    c2->SaveAs("c_Both_PID_6_GeV_e_pi_mu_n_"+title+"_cut.png");
  }
...
void drawCorrelation(TString particle, vector<TString> varnames, TH2F * hA, bool save=false) {
  ...
  auto c_A = new TCanvas("c_Both_PID_6_GeV_e_pi_mu_n_Correlation_"+particle,"c_Both_PID_6_GeV_e_pi_mu_n_Correlation_"+particle,800,800);
  c_A->cd();
  ...
  addProduction();
  if(save == true){
    c_A->SaveAs("c_Both_PID_6_GeV_e_pi_mu_n_correlation_"+particle+".eps");
    c_A->SaveAs("c_Both_PID_6_GeV_e_pi_mu_n_correlation_"+particle+".png");
  }
}
...
void drawCorrelationPlots(TH2F * hA, TH2F * hB, TH2F * hC, TH2F * hD, bool save = false) {
  ...
  TH2F * D_cor = new TH2F("n_correlation","n sample correlation between variables",nbins_x,-0.5,nbins_x+0.5,nbins_y,-0.5,nbins_y+0.5);
  ...
  drawCorrelation("n", varnames, D_cor, save);
  drawCorrelation("max", varnames, max_correlation, save);
}

void Performance_plots(bool save = false){

  TFile* outputfile=TFile::Open("../ROOTFiles/Both_PID_6_GeV_e_pi_mu_n_TMVA.root");
  ...
  drawLikelihood("e-likelihood", A_likelihood_for_A, A_likelihood_for_B, A_likelihood_for_C, A_likelihood_for_D, save);
  drawLikelihood("pi-likelihood", B_likelihood_for_A, B_likelihood_for_B, B_likelihood_for_C, B_likelihood_for_D, save);
  drawLikelihood("mu-likelihood", C_likelihood_for_A, C_likelihood_for_B, C_likelihood_for_C, C_likelihood_for_D, save); 
  drawLikelihood("n-likelihood", D_likelihood_for_A, D_likelihood_for_B, D_likelihood_for_C, D_likelihood_for_D, save);
