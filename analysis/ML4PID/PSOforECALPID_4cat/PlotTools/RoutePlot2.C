...
//       std::cout<<PrevTrees<<" "<<PrevShrinkage<<std::endl;
    ...
    for(size_t i=0;i<graphs.size();i++){
      ...
      leg->AddEntry(leggraphs.back(), Form("Partikel %i",i+1),"p");
    }
    TLegend* legMarker= new TLegend(0.7,0.55,0.9,0.9);
    ...
    TArrow* legArrow2 = new TArrow(1715,0.0311,1793,0.0311,0.02,">");
    ...
    for(size_t i=0;i<graphs.size();i++){
      ...
      if(i==0){
        ...
        emptygraph->SetTitle("Pfad der Partikel in der n_{Trees} - shrinkage Ebene");
        emptygraph->GetXaxis()->SetTitle("Anzahl der Bäume (n_{Trees})");
        emptygraph->GetYaxis()->SetTitle("Lernrate (shrinkage)");
        emptygraph->GetYaxis()->SetTitleOffset(1.5);
        emptygraph->GetXaxis()->SetTitleOffset(1.2);
//         emptygraph->GetXaxis()->SetTitleX(0.1);
